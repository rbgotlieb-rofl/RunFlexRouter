...
import {
  ...
  Locate, AlertTriangle, ArrowUp, CornerDownRight, RotateCw, Upload, Check, Loader2
} from 'lucide-react';
...
export default function LiveRunTracker({ route, onClose }: LiveRunTrackerProps) {
  ...
  const [stravaUploadState, setStravaUploadState] = useState<'idle' | 'uploading' | 'done' | 'error'>('idle');
  ...
  const { data: stravaStatus } = useQuery<{ configured: boolean; connected: boolean }>({
  ...
  const handleStravaUpload = async () => {
    if (stravaUploadState === 'uploading' || stravaUploadState === 'done') return;
    setStravaUploadState('uploading');
    try {
      const positions = positionsRef.current.map(p => ({
        lat: p.lat,
        lng: p.lng,
        time: p.timestamp ?? Date.now(),
      }));
      const res = await authFetch('/api/strava/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: `RunFlex: ${route.name.replace(/\s*\(.*\)/, '')}`,
          description: `Route generated by RunFlex`,
          elapsedSeconds: stats.elapsedSeconds,
          distanceMeters: stats.distanceKm * 1000,
          startDate: (runStartTimeRef.current ?? new Date()).toISOString(),
          positions,
        }),
      });
      if (!res.ok) throw new Error('Upload failed');
      setStravaUploadState('done');
    } catch {
      setStravaUploadState('error');
    }
  };
            ...
            <div className="w-full">
              ...
              {stravaStatus?.connected && (
                <Button
                  ...
                  disabled={stravaUploadState === 'uploading' || stravaUploadState === 'done'}
                  className="w-full py-5 text-lg mb-2 bg-[#FC4C02] hover:bg-[#e04400]"
                >
                  {stravaUploadState === 'uploading' && <Loader2 className="h-5 w-5 mr-2 animate-spin" />}
                  {stravaUploadState === 'done' && <Check className="h-5 w-5 mr-2" />}
                  {stravaUploadState === 'idle' && <Upload className="h-5 w-5 mr-2" />}
                  {stravaUploadState === 'error' && <Upload className="h-5 w-5 mr-2" />}
                  {stravaUploadState === 'idle' && 'Upload to Strava'}
                  {stravaUploadState === 'uploading' && 'Uploading...'}
                  {stravaUploadState === 'done' && 'Uploaded to Strava'}
                  {stravaUploadState === 'error' && 'Retry Upload'}
                </Button>
              )}
              <Button onClick={handleClose} className="w-full py-5 text-lg" variant={stravaStatus?.connected ? "outline" : "default"}>
                Done
              </Button>
            </div>
          )}
